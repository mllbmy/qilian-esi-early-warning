...
# ================= 标题 =================
para("祁连山区域生态安全预警评价及其驱动因子", 17, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, indent=False)
para("——基于 DPSIR–GM(1,1)–GeoDetector 的集成框架", 14, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, indent=False)
...
# ================= 摘要 =================
h1("摘要")
para("自然保护区对生物多样性保护至关重要，但其管理往往缺乏能将监测数据转化为可操作信号的定量预警工具。本研究构建了一个集成框架——融合驱动力-压力-状态-影响-响应（DPSIR）指标体系、主客观组合赋权（层次分析法 AHP 与熵权法）、灰色 GM(1,1) 模型与地理探测器（GeoDetector）方法——对中国祁连山区域（含祁连山国家公园）2005-2023 年的生态安全（ES）进行评价与预测。基于真实气候数据（CRU TS）与土地覆被数据（中国土地覆被数据集，CLCD）构建了涵盖九个指标、覆盖 DPSIR 各准则层的生态安全指数（ESI）；组合权重以 α=0.4 为基线并进行了全区间（α=0-1）敏感性分析。结果表明：（i）区域 ESI 在 0.26-0.71 之间波动，2010 年达峰值 0.709（较安全），2023 年降至 0.486（轻警）；（ii）19 年中有 16 年预警等级稳定为“轻警”，且 2023 年等级对权重比例 α 保持不变，证明评价结果具有稳健性；（iii）GM(1,1)、NGBM（非线性灰色伯努利模型）与 ARIMA 对 2024-2028 年的预测均指示轻警状态并呈缓慢下降趋势，其中 NGBM 优于 GM(1,1)，ARIMA 表现最差；（iv）地理探测器识别出裸地/退化占比（q=0.80）与草地占比（q=0.71）为空间生态安全异质性的主导因子。基于上述发现，我们提出分区管理建议，包括加强轻警区草地恢复、规范耕地扩张。该框架可推广至其他自然保护区，为保护区尺度的生态预警提供了可复现的基础。")
...
h2("1.3  本文贡献")
para("（1）构建了面向国家自然保护区、含单调性诊断与 NGBM/ARIMA 稳健性对比的 DPSIR–组合赋权–GM(1,1)–GeoDetector 集成框架；（2）进行了权重比例敏感性分析（α=0-1），证明预警等级对主客观赋权平衡具有稳健性；（3）为祁连山区域（含祁连山国家公园）2005-2023 年提供了真实数据证据链（时间趋势、稳健性、预测、空间驱动因子）及 2024-2028 五年预测；（4）提出了分区管理建议，并提供完全可复现的数据与代码。")
...
para("表 5 对比了三个模型的精度与预测结果。NGBM 达到合格精度（C=0.44，P=0.89），优于 GM(1,1)（C=0.60，勉强合格），ARIMA 表现最差（C=1.09，不合格），验证了非线性灰色模型对小样本、非平稳保护区尺度生态安全序列的适用性 (Althobaiti and Shabri, 2022)。所有预测均指示轻警状态并呈缓慢下降趋势（NGBM：0.446→0.394），提示若不干预将存在持续退化风险；参数化 95% 预测区间显示 NGBM 2024 年预测为 0.446（区间 0.358-0.533）、2028 年为 0.394（区间 0.306-0.482），区间跨度覆盖中警至较安全，预警等级存在一定不确定性。")
h2("4.4  权重比例 α 的稳健性")
...
h2("5.2  与既有研究对比")
para("α 敏感性分析证明预警等级对权重比例不敏感，支持了组合赋权方法的可靠性。预测对比表明 NGBM 比线性 GM(1,1) 更能捕捉非单调 ESI 动态，而 ARIMA 不适用于短小非平稳序列 (Althobaiti and Shabri, 2022)。与既有 DPSIR 评价研究 (Liu et al., 2023; Hou et al., 2024; Yang et al., 2023) 相比，本文框架新增：（i）显式权重比例稳健性检验；（ii）含单调性诊断的三模型预测对比；（iii）结合交互探测的空间因子归因（表 8）。")
...
# ================= 6 结论 =================
h1("6  结论")
para("本文将 DPSIR–组合赋权–GM(1,1)–GeoDetector 集成框架应用于祁连山区域（含祁连山国家公园），全部采用真实开放数据，管线完全可复现。2005-2023 年区域 ESI 介于 0.26-0.71；2011 年以来区域持续处于轻警状态（2023 年 ESI=0.486），Mann-Kendall 检验表明近年下降趋势不显著（p=0.263），生态安全呈稳定但脆弱的态势。")
